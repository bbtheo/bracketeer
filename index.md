...
Stage types are the verbs. Chain them onto
[`tournament()`](https://bbtheo.github.io/bracketeer/reference/tournament.md)
to describe any competition structure. The `from =` argument defaults to
the previous stage, so linear chains need no wiring at all.
