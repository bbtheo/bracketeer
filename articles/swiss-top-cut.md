...
`single_elim` stage implicitly reads from `swiss` via
...
This is the entire routing definition: 16 teams play a 5-round Swiss;
the top 8 by Swiss standings advance to a single-elimination bracket.
...
After the final Swiss result, the top-cut stage materializes
automatically.
...
(e.g., after a broadcast break), use `auto_advance = FALSE`:
...
accepts a data frame for entering multiple results at once.
